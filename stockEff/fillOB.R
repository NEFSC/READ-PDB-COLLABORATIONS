...
fillOB <- function(species_itis = NULL,
  ...
  if(is.null(tab_regions$stat_area) == FALSE){ # Stat areas grouped by region (default option)
    ...
    for(iregion in 1:length(tab_regions$stat_areas)){ # Set unique region ID tied to different stat_areas in each region and distinct area_fished
      ...
      AREA_FISHED <- c(AREA_FISHED, rep(area_fished[[iregion]], length(tab_regions$stat_areas[[iregion]])))
    }
    region_table <- cbind(AREA, REGION_ID, AREA_FISHED) %>% as.data.frame()
    
    tab_REGIONS <- NULL
    for(icams in 1:2){ # Loop over pre- and post-CAMS gear definitions
      temp_region <- expand.grid(AREA = unlist(tab_regions$stat_areas), YEAR = data_years, NESPP4_FLT = tab_nespp4_flt[[icams]]$nespp4_FLT) %>% mutate(AREA = as.character(AREA)) %>%
        dplyr::full_join(., region_table, by = "AREA", relationship = "many-to-many") %>% # Join expansion with region table to get IDs associated with stat_areas
        dplyr::distinct() %>% # Remove duplicate rows introduced by join
        cbind(SPECIES_ITIS = species_itis,
              ...
              SA_YEAR = assessment_year) %>%
          as.data.frame() %>% 
        relocate(c(YEAR, NESPP4_FLT, AREA, AREA_FISHED, REGION_ID), .after = SA_YEAR) #!!! need to test with multiple regions, code works but not sure if all region 1 should be together followed by all region 2 for subsequent processing
  ...
  if(tab_blocks$block_opt == "MONTH") { # Implement monthly blocks for each market category and year
    ...
    if(tab_blocks$check_missing == TRUE){ # Checks if region is missing for any years
      regions_missing <- checkLengths %>% filter(is.na(REGION_ID) == TRUE) %>% select(YEAR, MONTH, QTR, AREA, LENGTH = LENANML, NO_AT_LENGTH = NUMLEN, TRIPID, HAULNUM, LINK1, LINK3, PROGRAM)
    ...
    if(tab_blocks$block_opt == "checkLENGTHS"){
      ...
      defineBlocks <- expand.grid(YEAR = data_years,
        ...
        left_join(.,{checkLengths %>% group_by(YEAR, NESPP4_FLT, QTR) %>% mutate(NESPP4_FLT = as.numeric(NESPP4_FLT)) %>% summarise(LENGTHS_QTR = sum(NUMLEN))}) %>%
        left_join({checkLengths %>% group_by(YEAR, NESPP4_FLT, SEM) %>% mutate(NESPP4_FLT = as.numeric(NESPP4_FLT)) %>% summarise(LENGTHS_SEM = sum(NUMLEN))}) %>%
        left_join({checkLengths %>% group_by(YEAR, NESPP4_FLT) %>% mutate(NESPP4_FLT = as.numeric(NESPP4_FLT)) %>% summarise(LENGTHS_ANN = sum(NUMLEN))}) %>%
        ...
        group_by(YEAR, NESPP4_FLT, SEM, QTR) %>%
