...
if(multiWHAM[2] == TRUE){
  ...
  catch_proj <- sdrep[grep("log_catch", rownames(sdrep), fixed = TRUE),] %>% exp() #!!! need to test
...
BRPs <- prior_results$brps %>% mutate(source = "prior") %>% full_join(., MT_BRP)
...
write.csv(return_list$comb.model.summary, file = paste0(outdir, "/model_results.csv")) # Write SSB/F/R to .CSV
