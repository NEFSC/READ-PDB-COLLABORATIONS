...
mydat <- rbind(srcurve, constantrperssb) |>
...
$$
R = \frac{\alpha ~ SSB ~ e^{\theta_1 X}}{\beta ~ e^{\theta_2 X} ~ + ~ SSB ~ e^{\theta_3 X}}
$$ {#eq-cmlsr}
...
apply_thetas <- function(thetas, X, alpha, beta, ssb){
  recruits <- alpha * ssb * exp(thetas[1]*X) / (beta * exp(thetas[2]*X) + ssb * exp(thetas[3]*X))
  return(recruits)
}
...
calc_steepness <- function(thetas, X, alpha, beta, spr0, runname){
  cval <- exp(thetas * X)
  SSB0 <- (alpha * spr0 * cval[1] - beta * cval[2]) / cval[3]
...
calc_sr_depensation <- function(alpha, beta, ssb_breakpoint, depen_beta, ssb){
  r_breakpoint <- alpha * ssb_breakpoint / (beta + ssb_breakpoint)
  depen_alpha <- r_breakpoint / (ssb_breakpoint^depen_beta)
  recruits_depen <- ifelse(ssb <= ssb_breakpoint, depen_alpha*ssb^depen_beta, alpha * ssb / (beta + ssb))
...
ggplot(dfrl, aes(x=SSB, y=Recruits)) +
  geom_point() +
  facet_wrap(~stock, scales="free", ncol=3) +
  expand_limits(x = 0, y = 0) +
  theme_bw()
...
- [ ] Optional PDB Lunchtime Chat (September 4)
- [ ] Respond to feedback from PDB
